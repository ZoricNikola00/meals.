...
const fetchAreaMeals=async(str:string|undefined)=>{
  const data=await axios.get(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${str}`)
...
const AreaMeal = () => {
  const {str}=useParams()
  const {data,isLoading}=useQuery(['category',str],()=>fetchAreaMeals(str))
  return (
    <>
